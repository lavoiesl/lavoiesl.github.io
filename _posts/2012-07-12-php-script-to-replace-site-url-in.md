...
if (empty($argv[1]) || empty($argv[2])) {
    $log("Usage: " . basename(__FILE__) . " http://old-domain.com https://new-domain.com < database.orig.sql > database.new.sql\n");
    $log("    or " . basename(__FILE__) . " database.orig.sql https://new-domain.com > database.new.sql\n");
    exit(1);
}
...
if (is_file($argv[1])) {
    ...
    if (!$old_url) {
        $log("$input is not a valid Wordpress dump\n");
        exit(1);
    }
} else {
    $old_url = $argv[1];
...
fclose($ferr);
{% endhighlight %}
[View Gist](https://gist.github.com/lavoiesl/2227920) 
