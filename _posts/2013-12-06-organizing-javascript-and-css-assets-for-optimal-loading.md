...
However, be careful not to overload the browser. Keep it mind that the Javascript will be executed on each page load. For example, to not try to initialize every modal window just in case one might pop up. See [Optimizing page loads by reducing the impact of the Javascript initialization](/2013/12/optimizing-page-loads-by-reducing-javascript) for more details.
